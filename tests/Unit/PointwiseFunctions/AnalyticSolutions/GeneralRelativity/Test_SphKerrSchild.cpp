...
void test_sph_kerr_schild(const DataType& used_for_size) noexcept {
  ...
  const std::array<double, 3> spin{{.2, .3, .4}};
  const std::array<double, 3> center{{.0, 1., 2.}};
  ...
  auto a_squared = make_with_value<Scalar<DataType>>(x, 0.);
  for (size_t i = 0; i < 3; i++) {
  ...
  auto expected_matrix_Q =
      make_with_value<tnsr::Ij<DataType, 3, Frame>>(x, 0.0);
  for (size_t i = 0; i < 3; ++i) {
