...
// Definition of the IntermediateComputer constructor
...
// the spatial coordinates minus the center
template <typename DataType, typename Frame>
void SphKerrSchild::IntermediateComputer<DataType, Frame>::operator()(
  ...
  *x_sph_minus_center = x_;
  // get each component of x_sph_minus_center and subtract each by the center
  // coords
  for (size_t d = 0; d < 3; ++d) {
    x_sph_minus_center->get(d) -= gsl::at(solution_.center(), d);
  }

  // std::cout << "this is x_sph_minus_center " << "\n" << *x_sph_minus_center
  // << "\n";
}
...
void SphKerrSchild::IntermediateComputer<DataType, Frame>::operator()(
  ...
  // call each component of x_sph_minus_center and square then store it in
  // a Scalar<DataType> , e.g. (1,1,1)+(2,2,2)+(4,4,4)=(21,21,21)
  r_squared->get() = square(x_sph_minus_center.get(0));
  // std::cout << "outside of the loop: " << *r_squared << "\n";
  for (size_t i = 1; i < 3; ++i) {
    r_squared->get() += square(x_sph_minus_center.get(i));
    // std::cout << "inside of the loop: " << *r_squared << "\n";
  }
...
void SphKerrSchild::IntermediateComputer<DataType, Frame>::operator()(
  ...
  // "Basically Tensor<ANY_TYPE> + double is fine" - Nils
...
// Helper matrix for computing the Jacobian
template <typename DataType, typename Frame>
void SphKerrSchild::IntermediateComputer<DataType, Frame>::operator()(
...
// Helper matrix for computing the derivative of the Jacobian
template <typename DataType, typename Frame>
void SphKerrSchild::IntermediateComputer<DataType, Frame>::operator()(
...
// This is the derivative of the Jacobian
template <typename DataType, typename Frame>
void SphKerrSchild::IntermediateComputer<DataType, Frame>::operator()(
...
// Helper matrix for computing the inverse Jacobian
template <typename DataType, typename Frame>
void SphKerrSchild::IntermediateComputer<DataType, Frame>::operator()(
...
#define INSTANTIATE(_, data)                                       \
  template class SphKerrSchild::IntermediateComputer<DTYPE(data),  \
                                                     FRAME(data)>; \
